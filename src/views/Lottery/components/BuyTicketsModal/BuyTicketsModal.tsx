...
const BuyTicketsModal: React.FC<BuyTicketsModalProps> = ({ onDismiss }) => {
  ...
  useEffect(() => {
    const getMaxPossiblePurchase = () => {
      const maxBalancePurchase = memoisedUserCake.div(priceTicketInBpt)
      const limitedMaxPurchase = limitNumberByMaxTicketsPerBuy(maxBalancePurchase)
      let maxPurchase

      // If the users' max BPT Balance purchase is less than the contract limit - factor the discount logic into the max number of tickets they can purchase
  ...
  useEffect(() => {
    ...
    const costBeforeDiscount = priceTicketInBpt.times(numberOfTicketsToBuy)
  ...
  }, [ticketsToBuy, priceTicketInBpt, discountDivisor, getTicketCostAfterDiscount])
  ...
  const getErrorMessage = () => {
    if (userNotEnoughCake) return t('Insufficient BPT Balance')
  ...
  return (
    <StyledModal title={t('Buy Tickets')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <BalanceInput
        ...
        currencyValue={
          cakePriceBusd.gt(0) &&
          `~${ticketsToBuy ? getFullDisplayBalance(priceTicketInBpt.times(new BigNumber(ticketsToBuy))) : '0.00'} BPT`
        }
      />
      ...
      <Flex flexDirection="column">
        <Flex mb="8px" justifyContent="space-between">
          ...
          <Text color="textSubtle" fontSize="14px">
            {priceTicketInBpt && getFullDisplayBalance(priceTicketInBpt.times(ticketsToBuy || 0))} BPT
          </Text>
        </Flex>
        <Flex mb="8px" justifyContent="space-between">
          <Flex>
