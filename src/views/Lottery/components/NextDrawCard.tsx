...
const NextDrawCard = () => {
  ...
  const { endTime, amountCollectedInBpt, userTickets, status } = currentRound
  ...
  const prizeInBusd = amountCollectedInBpt.times(cakePriceBusd)
  const endTimeMs = parseInt(endTime, 10) * 1000
  const endDate = new Date(endTimeMs)
  const isLotteryOpen = status === LotteryStatus.OPEN
  const userTicketCount = userTickets?.tickets?.length || 0
  console.log('debug user ticket', userTicketCount, userTickets)
      ...
      <>
        ...
        {prizeInBusd.isNaN() ? (
          ...
          <Balance
            ...
            textAlign={['center', null, null, 'left']}
            unit=" BPT"
            value={getBalanceNumber(amountCollectedInBpt)}
            decimals={0}
          />
        )}
      </>
